...
class Biblioteca {
  ...
  // Metodos para los libros
    ...
    agregarEvento(evento: EventoBiblioteca) {
        ...
        // avisa a los socios registrados
        evento.sociosRegistrados.forEach(id => {
        const socio = this.buscarSocio(id);
        if (socio) {
            socio.agregarAviso(`Tienes un evento próximo: ${evento.nombre} el ${evento.fecha.toLocaleDateString()}`);
        }
        });
    }
  ...
  //Intenta agregar la reserva. Si es exitosa, muestra un mensaje; si no, indica que el socio ya reservó ese libro.
    devolverLibro1(socioId: number, libroISBN: string) {
        ...
        if (siguienteSocio) {
            ...
            siguienteSocio.agregarAviso(mensaje);
        }
        }
    }
  // Funciones de socios
    ...
    retirarLibro(socioId: number, libroISBN: string): void {
        ...
        // Bloquear préstamo si tiene una  deuda
