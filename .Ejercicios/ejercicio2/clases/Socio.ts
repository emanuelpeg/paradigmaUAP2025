...
import { PrestamoBase, PrestamoFactory, TipoPrestamo, Prestamo } from "./PrestamoBase";

/** Duracion en dias de un prestamo */
export type Duracion = number;

export abstract class Socio {
  protected prestamos: PrestamoBase[] = [];
  ...
  get getPrestamos() { return this.prestamos; }

  abstract getDuracionPrestamo(): Duracion;
  abstract getMaximoLibros(): number;

  retirarLibro(prestamo: PrestamoBase): void {
    if (this.tienePrestadoLibro(prestamo.getLibro)) throw new Error(`Ya tienes el libro ${prestamo.getLibro.titulo} prestado.`);
    if (!this.puedeRetirar()) throw new Error("No tiene permisos para retirar este libro");
    this.prestamos.push(prestamo);
  }

  devolver(libro: Libro) {
    const prestamo = this.tienePrestadoLibro(libro);
    if (!prestamo) throw new Error("No esta prestado");
    const indice = this.prestamos.indexOf(prestamo);
    this.prestamos.splice(indice, 1);
    return prestamo;
  }

  tienePrestadoLibro(libro: Libro): PrestamoBase | null {
...
export class SocioRegular extends Socio {
  ...
  devolver(libro: Libro): PrestamoBase {
    // Manejar potenciales multas
    const prestamo = this.tienePrestadoLibro(libro);
    if (!prestamo) throw new Error('El préstamo no existe.')
    if (prestamo.estaVencido()){
      throw new Error(`El préstamo está vencido por ${prestamo.diasRetraso}. Multa de ${prestamo.calcularMulta()}`);
    }
