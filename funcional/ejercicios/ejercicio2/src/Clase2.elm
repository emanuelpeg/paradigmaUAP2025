...
buscar lista com =
    case lista of
        [] -> 0
        x :: xs -> List.foldl (\y acum -> if com y acum then y else acum) x xs
...
mediana lista =
    ...
    in
    if len == 0 then 0
    else obtenerElemento ordenada (len // 2)


-- Cuenta los elementos

contar : List Int -> Int
contar lista = List.length lista


-- Acumula los elementos

acc : List Int -> Int
acc lista = List.sum lista
...
removerDuplicados lista =
    List.reverse (List.foldl (\x accList -> if List.member x accList then accList else x :: accList) [] lista)
